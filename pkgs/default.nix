{ pkgs ? null, final ? null, prev ? null, rustPlatform ? null }:
...
let
  ...
  pkgsOverrides = if rustPlatform != null then {
    inherit rustPlatform;
  } else {};
  callPackage = if final != null then
    (path: args: let
      drvFunc = import path;
      drvArgs = builtins.functionArgs drvFunc;
      drvArgsOverrides = builtins.intersectAttrs drvArgs pkgsOverrides;
    in final.callPackage drvFunc (drvArgsOverrides // args))
  else
    pkgs.lib.callPackageWith (pkgs // pkgsOverrides // finalPkgs // { buildPackages = finalPkgs; targetPackages = finalPkgs; inherit callPackage; });
